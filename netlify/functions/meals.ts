...
import { persons, mealEntries } from "../../db/schema.js";
...
async function getDay(dateStr: string) {
  ...
  const existing = await db
    .select()
    .from(mealEntries)
    .where(eq(mealEntries.entryDate, dateStr));
  ...
  return { date: dateStr, mealCost: MEAL_COST, grid };
}
...
async function getSummary(start: string, end: string) {
  ...
  const rows = await db
    .select()
    .from(mealEntries)
    .where(and(gte(mealEntries.entryDate, start), lte(mealEntries.entryDate, end)));
